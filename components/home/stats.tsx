...
import { Users, BookOpen, FileText, Download } from 'lucide-react'

export function StatsSection({ stats }: { stats: { users: number; notes: number; pyqs: number; downloads: number } }) {
  const items = [
    ...
    { label: 'Downloads', value: stats.downloads, icon: Download, color: 'text-orange-600', bg: 'bg-orange-100 dark:bg-orange-900/30' },
  ]
  return (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {items.map((item, i) => (
            <motion.div key={item.label} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }} className="text-center">
              ...
              <div className="text-3xl font-bold tabular-nums">{item.value.toLocaleString()}+</div>
