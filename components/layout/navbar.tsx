...
import { BookOpen, FileText, Calculator, Crown, Menu, X, Sun, Moon, LogOut, User, LayoutDashboard, ChevronDown, Shield, ShoppingBag, Bug } from 'lucide-react'
                ...
                <AnimatePresence>
                  {userMenu && (<>
                    ...
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.15 }} className="absolute right-0 mt-2 w-52 bg-background border rounded-xl shadow-xl z-20 overflow-hidden py-1">
                      ...
                      {[{href:'/dashboard',label:'Dashboard',icon:LayoutDashboard},{href:'/profile',label:'Profile',icon:User},{href:'/merchandise/my-listings',label:'My Listings',icon:ShoppingBag},{href:'/my-reports',label:'My Reports',icon:Bug},...(user?.role==='ADMIN'?[{href:'/admin',label:'Admin Panel',icon:Shield}]:[])].map(item=>(
