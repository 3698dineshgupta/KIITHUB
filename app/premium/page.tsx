...
import { Crown, Check, Upload, Loader2, AlertCircle, Sparkles, Calendar, Clock, ShieldCheck } from 'lucide-react'
...
export default function PremiumPage() {
  ...
  const [msgPending, setMsgPending] = useState('Your payment is being verified. Activation within 24 hours.')
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (mounted) {
          if (data?.bankQrCode) setBankQrCode(String(data.bankQrCode))
          ...
          if (data?.currency_code) setCurrencyCode(String(data.currency_code))
          if (data?.premium_days) setPremiumDays(String(data.premium_days))
          if (data?.upi_id) setUpiId(String(data.upi_id))
          if (data?.payment_instructions) setPaymentInstructions(String(data.payment_instructions))
          if (data?.message_pending) setMsgPending(String(data.message_pending))
          
          if (data?.premium_features) {
            const list = String(data.premium_features)
              .split('\n')
              .map(x => x.trim())
              .filter(Boolean)
            setPremiumFeatures(list)
          }
        }
      } catch (e) {
        console.error('Failed to load dynamic premium settings:', e)
      }
    })()
